...
export class IniciarDiagnosticoUseCase {
	private readonly logger = new Logger(IniciarDiagnosticoUseCase.name);
	...
	async execute(id: string, usuarioId: string): Promise<IServiceResponse<unknown>> {
		...
		if (!canTransition(os.status, "iniciar_diagnostico"))
			return SR.unprocessableEntity(undefined, `Transição inválida a partir do status ${os.status}`);

		const novo = nextStatus("iniciar_diagnostico");
